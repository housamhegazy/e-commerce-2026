...
const authSlice = createSlice({
  ...
  extraReducers: (builder) => {
    ...
    //   },
    // );
// 2. إضافة منطق تحديث الـ Wishlist
    builder.addMatcher(
      productApi.endpoints.addToWishlist.matchFulfilled,
      (state, action) => {
        // action.payload هو الرد اللي راجع من السيرفر
        // لو السيرفر بيرجع الـ wishlist الجديدة مباشرة:
        if (state.user && action.payload.wishlist) {
