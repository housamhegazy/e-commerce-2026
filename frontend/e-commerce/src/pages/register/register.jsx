...
const Register = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    if (formData.password !== formData.confirmPassword) {
      return alert("Passwords do not match!"); // ممكن تستخدم Toast هنا
    }

    try {
      // 2. إرسال البيانات (بنبعت اللي السيرفر محتاجه بس)
      const { name, email, password } = formData;
      await signup({ name, email, password }).unwrap();

      // 3. التوجيه في حالة النجاح
      navigate("/");
    } catch (error) {
      // التعامل مع أخطاء السيرفر (مثل إيميل موجود مسبقاً)
    ...
    >
      <div className="row w-100 justify-content-center">
        <div className="col-12 col-md-6 col-lg-4">
          ...
          <div
            className="p-5 rounded-4 shadow-lg border border-secondary border-opacity-10"
            style={{
          ...
          >
            ...
            <form onSubmit={handleSubmit}>
              ...
              <button
                type="submit"
                className="btn btn-warning w-100 fw-bold py-3 mb-3 border-0 shadow-sm"
                style={{
              ...
              <div className="text-center">
                ...
                <Link
                  to="/login"
                  className="text-warning small fw-bold text-decoration-none"
                >
                  Login
                </Link>
              </div>
